...
class SoftProjection(nn.Module):
    ...
    def _group_points(self, point_cloud, query_cloud, point_features=None):
        ...
        grouped_points = group_point(point_cloud, idx.contiguous())  # B x 3 x QueryPoints x K
        grouped_features = (
    ...
    def update_temperature(self, epoch, tot_epochs):
        self._temperature.data = torch.tensor(self.initial_temperature * math.exp(-epoch * 10 / tot_epochs),
                                              dtype=torch.float32)
